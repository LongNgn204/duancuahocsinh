    ...
    <div className="max-w-3xl mx-auto space-y-6 pb-8">
      ...
      >
        ...
        <Card variant="outlined" className="border-red-500/30 bg-red-500/5">
          <Card.Header>
            <Card.Title className="text-red-500">⚠️ Vùng nguy hiểm</Card.Title>
            <Card.Description>Các hành động này không thể hoàn tác</Card.Description>
          </Card.Header>
          <Card.Content>
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 rounded-xl bg-red-500/10 border border-red-500/20">
                <div>
                  <h4 className="font-medium text-[--text]">Xóa tất cả dữ liệu</h4>
                  <p className="text-sm text-[--muted]">Xóa toàn bộ dữ liệu và cài đặt</p>
                </div>
                <Button
                  variant="danger"
                  size="sm"
              ...
              {isLoggedIn() && (
                <div className="flex items-center justify-between p-3 rounded-xl bg-red-600/10 border border-red-600/30">
                  <div>
                    <h4 className="font-medium text-red-600">Xóa tài khoản</h4>
                    <p className="text-sm text-[--muted]">
                      Xóa vĩnh viễn tài khoản và tất cả dữ liệu trên server
                    </p>
                  </div>
                  <Button
                    variant="danger"
                    size="sm"
                    onClick={async () => {
                      if (!confirm('⚠️ CẢNH BÁO: Bạn có chắc muốn XÓA TÀI KHOẢN?\n\nTất cả dữ liệu sẽ bị xóa vĩnh viễn:\n- Tài khoản\n- Nhật ký\n- Lọ Biết Ơn\n- Thành tích\n- Thống kê\n- Tất cả dữ liệu khác\n\nHành động này KHÔNG THỂ hoàn tác!')) {
                        return;
                      }
                      if (!confirm('Bạn thực sự chắc chắn? Nhập "XÓA TÀI KHOẢN" để xác nhận (chỉ cần xác nhận).')) {
                        return;
                      }
                      setIsDeletingAccount(true);
                      try {
                        await deleteAccount();
                        alert('Tài khoản đã được xóa thành công. Ứng dụng sẽ tải lại.');
                        // Clear local storage
                        localStorage.clear();
                        // Reload page
